// From: https://github.com/mvdnes/spin-rs/blob/master/src/mutex.rs
#![allow(unused)]
...
trait CompareAndSwap {
    type T;
    fn compare_and_swap(&self, current: bool, new: bool, ordering: Ordering) -> bool;
}
impl CompareAndSwap for AtomicUsize {
    type T = usize;
    fn compare_and_swap(&self, current: bool, new: bool, ordering: Ordering) -> bool {
        if current == new {
        ...
        let ptr = self as *const AtomicUsize as *const usize;
        unsafe {
            asm!("amoswap.w $0, $1, ($2)" : "=r"(previous) : "r"(new), "r"(ptr) :: "volatile")
        }
        previous != 0
    }
}

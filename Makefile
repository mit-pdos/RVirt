...
qemu: debug
	qemu-system-riscv64 -machine virt -kernel target/riscv64imac-unknown-none-elf/debug/svpn -nographic -initrd fedora-vmlinux -m 1G \
...
qemu-release: release
	qemu-system-riscv64 -machine virt -kernel target/riscv64imac-unknown-none-elf/release/svpn -nographic -initrd fedora-vmlinux -m 1G \
	    -object rng-random,filename=/dev/urandom,id=rng0 \
	    -device virtio-rng-device,rng=rng0 \
	    -append "console=ttyS0 ro root=/dev/vda" \
	    -device virtio-blk-device,drive=hd0 \
